...
def _load_vggt_state_dict(vggt_checkpoint_path=None):
    if vggt_checkpoint_path:
        ckpt_path = Path(vggt_checkpoint_path).expanduser().resolve()
        if not ckpt_path.is_file():
            raise FileNotFoundError(f"VGGT checkpoint not found: {ckpt_path}")
        print(f"Loading VGGT checkpoint from local path: {ckpt_path}")
        return torch.load(str(ckpt_path), map_location="cpu")

    print("Downloading/loading VGGT checkpoint from HuggingFace: facebook/VGGT-1B")
    ckpt_path = hf_hub_download("facebook/VGGT-1B", filename="model.pt")
...
def create_target_camera_path(
    ...
    Args:
        ...
        vggt_checkpoint_path: Optional local VGGT checkpoint path. If empty,
            falls back to HuggingFace download/cached file.
    ...
    vggt_pretrained_state = _load_vggt_state_dict(vggt_checkpoint_path)
